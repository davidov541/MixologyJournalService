...
const uut = rewire('../../deleterecipe/deleterecipe')
...
describe('Delete Recipe Function Tests', function () {
    ...
    test('should not delete the recipe if authentication succeeds for a non-admin user.', async function () {
        const mockSecurity = setupMockSecurity()
    ...
    test('should correctly delete the recipe if authentication succeeds for an admin user.', async function () {
        const mockSecurity = setupMockSecurity()
        ...
        const expectations = [
            ...
            mockPersistence.expects("deleteEntry")
                .once()
                .withExactArgs(request.body.id, ['uses', 'derivative'])
        ]
